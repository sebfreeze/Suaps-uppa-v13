...
st.set_page_config(page_title="SUAPS UPPA V14 Mobile", page_icon="🏃", layout="centered", initial_sidebar_state="collapsed")
...
st.markdown('<div class="hero"><h1>SUAPS UPPA</h1><p>Bouge ton campus, révèle ton potentiel !</p></div>', unsafe_allow_html=True)

page = st.session_state.page

if page == "Accueil":
    st.subheader("Bonjour 👋")
    st.write("Ton sport, tes séances, tes résultats et ta progression au même endroit.")
    c1, c2 = st.columns(2)
    with c1:
        st.metric("Présences","24 / 28")
        st.metric("Compétences","72 %")
    with c2:
        st.metric("Moyenne","14,6 / 20")
        st.metric("Performances","12")
    st.info("🏊 **Natation — Groupe intermédiaire**\n\nMercredi 18h30 • Piscine universitaire")

elif page == "Activités":
    st.subheader("6 activités, 6 ambiances")
    for ico, title, desc in SPORTS:
        st.markdown(f'<div class="card"><div class="sport">{ico} {title}</div><div class="muted">{desc}</div></div>', unsafe_allow_html=True)
...
st.divider()
cols = st.columns(5)
labels = [("🏠","Accueil"),("🏅","Activités"),("📝","Inscription"),("✅","Présence"),("📊","Résultats")]
