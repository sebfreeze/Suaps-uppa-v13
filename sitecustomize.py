...
try:
    ...
    def _patch_suaps_generated_app(source):
        ...
        cex1.download_button("Inscriptions CSV",_ins_df.to_csv(index=False).encode("utf-8-sig"),file_name=f"inscriptions_{export_sem.split('—')[0].strip().lower().replace(' ','_')}_2026_2027.csv",mime="text/csv",use_container_width=True)\n        cex2.download_button("Présences CSV",_pre_df.to_csv(index=False).encode("utf-8-sig"),file_name=f"presences_{export_sem.split('—')[0].strip().lower().replace(' ','_')}_2026_2027.csv",mime="text/csv",use_container_width=True)\n        cex3.download_button("Évaluations CSV",_eva_df.to_csv(index=False).encode("utf-8-sig"),file_name=f"evaluations_{export_sem.split('—')[0].strip().lower().replace(' ','_')}_2026_2027.csv",mime="text/csv",use_container_width=True)\n    if st.button("← Accueil"): go("Accueil")'''
