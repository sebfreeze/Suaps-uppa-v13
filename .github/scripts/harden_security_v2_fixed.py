...
patch_generated = '''    # Expiration automatique des sessions étudiant + cloisonnement enseignants.
...
'''
if patch_generated not in text:
    if marker not in text:
        raise SystemExit("Ancre introuvable: cloisonnement enseignants")
    text = text.replace(marker, patch_generated + "\n" + marker, 1)
    print("sessions étudiant + groupes enseignant: appliqué")
...
new_admin_start = '''def admin_login():
    ...
    if st.session_state.get("admin_auth") and st.session_state.get("teacher_name"):
        ...
        _clear_teacher_auth()'''
text = replace_once(text, old_admin_start, new_admin_start, "expiration login enseignant")
...
text = replace_once(text, old_admin_guard, new_admin_guard, "expiration admin enseignant")
...
text = text.replace(
    ...
    '_clear_teacher_auth(); st.session_state.profil=None; go("Accueil")',1)
...
# -----------------------------------------------------------------------------
# Modèle/import Excel : identifiant réellement obligatoire
# -----------------------------------------------------------------------------
root_path = Path("sitecustomize.py")
root = root_path.read_text(encoding="utf-8")
root = root.replace(
    "Colonnes obligatoires : nom, prenom, email, activite, creneau. Pour la modalité, mettre X dans une seule colonne : UET, UECF ou Non noté. Recommandées : semestre, jour_horaire. Facultatives : identifiant, composante, profil, responsable, co_responsable.",
    "Colonnes obligatoires : nom, prenom, email, identifiant, activite, creneau. Pour la modalité, mettre X dans une seule colonne : UET, UECF ou Non noté. Recommandées : semestre, jour_horaire. Facultatives : composante, profil, responsable, co_responsable.",1)
root = root.replace('{"colonne":"identifiant","statut":"Facultatif"','{"colonne":"identifiant","statut":"Obligatoire"',1)
root = root.replace('required=["nom","prenom","email","activite","creneau"]','required=["nom","prenom","email","identifiant","activite","creneau"]',1)
root = root.replace(
    ...
    'nom=clean("nom"); prenom=clean("prenom"); email=clean("email").lower(); identifiant=clean("identifiant"); activite=clean("activite"); creneau=clean("creneau"); horaire=clean("jour_horaire")',1)
root = root.replace(
    'if not nom or not prenom or not email or not activite or not creneau:',
    'if not nom or not prenom or not email or not identifiant or not activite or not creneau:',1)
root_path.write_text(root, encoding="utf-8")

# -----------------------------------------------------------------------------
# Page multipage Pédagogie : même authentification + expiration
# -----------------------------------------------------------------------------
...
print("Renforcement sécurité V2 appliqué.")
