...
def _round_robin(team_ids):
    ...
    for rnd in range(len(ids) - 1):
        for i in range(len(ids) // 2):
            a, b = ids[i], ids[-1-i]
            if a is not None and b is not None: games.append((a, b, f"Journée {rnd+1}"))
...
def render_sports_co(st, rows, one, exe, date):
    ...
    if tab == "Équipes":
        ...
        with st.form("sc_tournament"):
            nom = st.text_input("Nom du tournoi"); formule = st.selectbox("Formule", ["Championnat / toutes rondes", "Poules", "Élimination directe"]); d = st.date_input("Date", date.today()); lieu = st.text_input("Lieu"); create = st.form_submit_button("Créer le tournoi", type="primary")
        ...
        if len(participants) >= 2 and st.button("Générer les rencontres", type="primary"):
            ...
            st.success(f"{len(games)} rencontre(s) générée(s)."); st.rerun()

    elif tab == "Classement":
        ts = rows("SELECT * FROM tournois WHERE activite=? ORDER BY id DESC", (sport,))
        if not ts: st.info("Aucun tournoi."); return
        t = st.selectbox("Tournoi", ts, format_func=lambda r:r["nom"], key="rank_t")
        teams = rows("SELECT e.* FROM tournoi_equipes te JOIN equipes e ON e.id=te.equipe_id WHERE te.tournoi_id=?", (t["id"],)); stats = {e["id"]:{"Équipe":e["nom"],"J":0,"G":0,"N":0,"P":0,"Pour":0,"Contre":0,"Pts":0} for e in teams}
        ...
        st.markdown(f"## 📋 {m['equipe_a']} — {m['equipe_b']}"); st.caption(f"{sport} • {m['date_match']} • {m['heure'] or ''} • {m['lieu'] or ''} • Arbitre : {m['arbitre'] or 'à définir'}")
        ca, cb = st.columns(2)
